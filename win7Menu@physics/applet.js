...
MyApplet.prototype = {
    ...
    _onMenuKeyPress: function (actor, event) {
        ...
        if (symbol == Clutter.KEY_Up) {
            if (this._activeContainer == this.categoriesBox) index = this._selectedItemIndex - 1 < 0 ? 0 : this._selectedItemIndex - 2;
            else index = this._selectedItemIndex - 1 < 0 ? 0 : this._selectedItemIndex - 1;
        } else if (symbol == Clutter.KEY_Down) {
            if (this._activeContainer == this.categoriesBox && this._selectedItemIndex != -1) index = this._selectedItemIndex + 2 >= children.length ? children.length - 2 : this._selectedItemIndex + 2;
...
//            item_actor._delegate.menu.toggle();
    ...
    _clearSelections: function (container) {
        container.get_children().forEach(function (actor) {
            ...
            if (actor._delegate && actor._delegate.menu)
                actor._delegate.menu.close(true);
        });
    },
    ...
    _appletStyles: function () {
            this.favsBox.style = "min-height: 455px;min-width: 235px;";
            this.favBoxWith = (this.favsBox.get_allocation_box().x2 - this.favsBox.get_allocation_box().x1);
            let searchEntryWidth = this.favBoxWith - this._searchInactiveIcon.get_theme_node().get_length('icon-size')
            ...
            this.categoriesScrollBox.set_style("width: " + this.favBoxWith + "px; height: " + scrollBoxHeight + "px;");
    },
    ...
    _refreshApps: function () {
        ...
        this._allAppsCategoryButton.actor.connect('button-press-event', Lang.bind(this, function () {
            this._categoryButtonsHidden(false);
            if (!this._allAppsCategoryButton.menu.isOpen)
                this._select_category(null, this._allAppsCategoryButton);
        }));
                ...
                Tweener.addTween(this, {
                    ...
                    onComplete: function () {
                        if (this._allAppsCategoryButton.isHovered) {
                            //this._select_category(null, this._allAppsCategoryButton);
                            this._allAppsCategoryButton.actor.add_style_class_name("menu-category-button-selected");
                        }
                    }
                });
            }
        ...
        this._allAppsCategoryButton.actor.connect('leave-event', Lang.bind(this, function () {
            ...
            if (!this._allAppsCategoryButton.menu.isOpen)
                this._allAppsCategoryButton.actor.remove_style_class_name("menu-category-button-selected");
        }));

        this.categoriesBox.add_actor(this._allAppsCategoryButton.actor);
        this.categoriesBox.add_actor(this._allAppsCategoryButton.menu.actor);
            ...
            while ((nextType = iter.next()) != GMenu.TreeItemType.INVALID) {
                if (nextType == GMenu.TreeItemType.DIRECTORY) {
                    ...
                    if (this.applicationsByCategory[dir.get_menu_id()].length > 0) {
                        ...
                        categoryButton.actor.connect('button-press-event', Lang.bind(this, function () {
                            if (!categoryButton.menu.isOpen) {
                                ...
                                Tweener.addTween(this, {
                                    ...
                                    onComplete: function () {
                                        if (categoryButton.isHovered) {
                                            //this._select_category(dir, categoryButton);
                                            categoryButton.actor.add_style_class_name("menu-category-button-selected");
                                        }
                                    }
                                });
                            }
                        ...
                        categoryButton.actor.connect('leave-event', function () {
                            ...
                            if (!categoryButton.menu.isOpen)
                                categoryButton.actor.remove_style_class_name("menu-category-button-selected");
                        });
                        this.categoriesBox.add_actor(categoryButton.actor);
                        this.categoriesBox.add_actor(categoryButton.menu.actor);
                    }
                }
            }
        }
    ...
    _display: function () {
        ...
        this.placesButton.actor.connect('button-press-event', Lang.bind(this, function () {
            if (!this.placesButton.menu.isOpen) {
                ...
                Tweener.addTween(this, {
                    ...
                    onComplete: function () {
                        if (this.placesButton.isHovered) {
                            //this._select_places(this.placesButton);
                            this.placesButton.actor.add_style_class_name("menu-category-button-selected");                                  
                        }
                    }
                });
            }
        ...
        this.placesButton.actor.connect('leave-event', Lang.bind(this, function () {
            ...
            this.placesButton.actor.remove_style_class_name("menu-category-button-selected");
        }));
    ...
    _select_category: function (dir, categoryButton) {
        ...
        if (dir) this._displayButtons(this._listApplications(dir.get_menu_id()), null, categoryButton);
        else this._displayButtons(this._listApplications(null), null, categoryButton);
        this.closeApplicationsContextMenus(null, false);
    },
    ...
    _displayButtons: function (apps, places, categoryButton) {
        if (apps) {
            for (var i = 0; i < apps.length; i++) {
                ...
                if (!this._applicationsButtons[app]) {
                    ...
                    applicationButton.actor.connect('leave-event', Lang.bind(this, function () {
                        this.selectedAppTitle.set_text("");
                        this.selectedAppDescription.set_text("");
                        applicationButton.actor.remove_style_class_name("menu-category-button-selected");
                    }));
                    this._addEnterEvent(applicationButton, Lang.bind(this, function () {
                        ...
                        //this._clearSelections(this.categoriesBox);
                        applicationButton.actor.add_style_class_name("menu-category-button-selected");
                    }));
                    this._applicationsButtons[app] = applicationButton;
                }
                categoryButton.menu.addMenuItem(this._applicationsButtons[app]);
            }
        }
        ...
        if (places) {
            for (var i = 0; i < places.length; i++) {
                ...
                this._addEnterEvent(button, Lang.bind(this, function () {
                    //this._clearSelections(this.categoriesBox);
                    button.actor.add_style_class_name("menu-category-button-selected");
                }));
                button.actor.connect('leave-event', Lang.bind(this, function () {
                    button.actor.remove_style_class_name("menu-category-button-selected");
                }));
                categoryButton.menu.box.add_actor(button.actor);
            }
        }
    },
    ...
    _select_places: function (button) {
        ...
        this._displayButtons(null, bookmarks.concat(devices), button);
    },

    _categoryButtonsHidden: function (hide) {
        let categoriesBoxChilren = this.categoriesBox.get_children();
        if (this.categorysHidden && !hide) {
            for (var i = 0; i < categoriesBoxChilren.length; i++) {
                let item_actor = categoriesBoxChilren[i];
                item_actor.show();
            }
            this.categorysHidden = false;
            this._allAppsCategoryButton.label.set_text(_("All Applications"));
        }else if (hide) {
            for (var i = 0; i < categoriesBoxChilren.length; i++) {
                let item_actor = categoriesBoxChilren[i];
                if (item_actor != this._allAppsCategoryButton.actor && item_actor != this._allAppsCategoryButton.menu.actor)
                    item_actor.hide();
            }
            this.categorysHidden = true;
            this._allAppsCategoryButton.label.set_text(_("Back to Categorys"));
        }
    ...
    _doSearch: function () {
        ...
        this._displayButtons(appResults, placesResults, this._allAppsCategoryButton);
        this._allAppsCategoryButton.menu.toggle();
        

        let categoriesBoxChilren = this.categoriesBox.get_children();
        if (categoriesBoxChilren.length > 0) {
            this._activeContainer = this.categoriesBox;
            this._selectedItemIndex = 0;
            let item_actor = categoriesBoxChilren[this._selectedItemIndex];
            if (item_actor && item_actor !== this.searchEntry && item_actor._delegate) {
                item_actor._delegate.emit('enter-event');
            }
        }
        this._categoryButtonsHidden(true);
